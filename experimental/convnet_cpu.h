...
class Edge {
 ...
 private:
  ...
  const float add_scale_, pow_scale_, num_filters_response_norm_ ;
...
class ConvNetCPU {
 public:
  ...
  void Fprop(const float* data, int batch_size);
  int GetDims(const string& layer_name) const;
  Layer* GetLayerByName(const string& name);
  void SetMean(const string& mean_file);

 private:
  void Normalize(const float* i_data, float* o_data, int num_dims, int num_colors);
  void Sort();
