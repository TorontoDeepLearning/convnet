...
int main(int argc, char** argv) {
  ...
  LinearLayer input(layer_config);
  int size = 28, batchsize = 128;
  input.AllocateMemory(size, batchsize);
  ...
  for (int batch = 0; batch < 100; batch++) {
    dh->GetBatch(layers);
    input.GetState().CopyToHost();

    int width = size;
    int height = size;
    CImgDisplay main_disp;
    float* data = input.GetState().GetHostData();
    CImg<float> img(size, size, 1, 3);
    int offset = 0;
    for (int k = 0; k < 3; k++) {
      for (int i = 0; i < size; i++) {
        for (int j = 0; j < size; j++) {
          img(j, i, 0, k) = data[offset + batchsize * (j + width * (i + k * height))];
        }
      }
    }
