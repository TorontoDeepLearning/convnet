...
class DataWriter {
 public:
  ...
  void SetDataSetSize(int dataset_size) { dataset_size_ = dataset_size; }
  virtual void Write(vector<Layer*>& layers, int numcases);
  void WriteHDF5(Matrix& m, const string& dataset, int numcases);

 private:
  int dataset_size_;
  typedef struct {
    int num_dims, current_row, average_batches, average_online, counter;
